...
// Loads one face, baked at FONT_BAKE_SIZE. Falls back to raylib's built-in face with
// a warning when the file is not there, so a missing font degrades the look rather
...
// Releases one face, unless it is the built-in fallback - unloading that would take
// down the face every other text call still depends on.
...
// The pixel rectangle one board cell occupies, shrunk by `inset` on every side. The
// inset is what leaves a visible gap between adjacent body segments.
Rectangle cellRect(const Position& cell, float inset)
{
    return { MARGIN + cell.x * static_cast<float>(CELL) + inset,
             HEADER_HEIGHT + cell.y * static_cast<float>(CELL) + inset, CELL - inset * 2.0f,
             CELL - inset * 2.0f };
}
...
{
    const float board_width = static_cast<float>(boardPixelWidth(columns));
    const float board_height = static_cast<float>(boardPixelHeight(rows));
    ...
    {
        float x = MARGIN + column * static_cast<float>(CELL);
        DrawLineV({ x, static_cast<float>(HEADER_HEIGHT) }, { x, HEADER_HEIGHT + board_height },
                  GRID_LINE);
    }
    for (int row = 1; row < rows; row++)
    {
        float y = HEADER_HEIGHT + row * static_cast<float>(CELL);
        DrawLineV({ static_cast<float>(MARGIN), y }, { MARGIN + board_width, y }, GRID_LINE);
    }
    ...
    {
        float along = body.size() > 1
                          ? static_cast<float>(index) / static_cast<float>(body.size() - 1)
                          : 0.0f;
        Color segment = { static_cast<unsigned char>(MINT.r + (MINT_DEEP.r - MINT.r) * along),
                          static_cast<unsigned char>(MINT.g + (MINT_DEEP.g - MINT.g) * along),
                          static_cast<unsigned char>(MINT.b + (MINT_DEEP.b - MINT.b) * along),
                          255 };

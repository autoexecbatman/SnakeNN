torch::Tensor SnakeNeuralNetworkImpl::getAction(const std::vector<float>& state, float epsilon) {
    try {
        ...
        if (std::uniform_real_distribution<float>(0.0f, 1.0f)(rng) < epsilon) {
            auto action = torch::randint(0, 4, {1}, torch::kInt64);  // FIXED: Use torch::kInt64 consistently
            logDebug("Random action selected: " + std::to_string(action.item<int64_t>()));  // FIXED: Use int64_t
            return action;
        }
        ...
        auto state_tensor = torch::zeros({1, static_cast<int64_t>(state.size())}, torch::kFloat);
        ...
        logDebug("Action selected: " + std::to_string(action.item<int64_t>()));
        
        return action;
    } catch (const std::exception& e) {
        logDebug("ERROR in getAction(): " + std::string(e.what()));
        // Return safe default action
        return torch::tensor({0}, torch::kInt64);  // FIXED: Use torch::kInt64 consistently
    }
}